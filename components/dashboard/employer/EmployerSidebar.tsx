...
function NavItem({
  ...
  // FIXED: More precise active state checking
  const isActive =
    pathname === item.href || // Exact match
    (item.href !== "/dashboard/employer" &&
      pathname.startsWith(item.href + "/")); // Child routes only for non-root items
  ...
  const inner = (
    ...
    >
      ...
      >
        ...
        <div
          style={{ margin: "16px 12px", borderTop: `1px solid ${S.border}` }}
        />
