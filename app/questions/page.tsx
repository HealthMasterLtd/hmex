      ...
      {Array.from({ length: Math.min(total, 15) }).map((_, i) => (
        <div
          ...
          style={{
            ...
            height: 6,
            backgroundColor: i < current ? '#0FBB7D' : i === current ? 'rgba(15,187,125,0.4)' : 'rgba(15,187,125,0.15)',
          }}
        />
      ))}
    </div>
...
export default function AssessmentPage() {
  ...
  const loadQuestion = async () => {
    ...
    try {
      ...
      if (!next) {
        ...
        holdAssessmentLocally(assessment, allAnswers);
        router.push("/review");
        return;
      }
  ...
  const progress = Math.min(100, Math.round((step / groqService.getMaxQuestions()) * 100));
  ...
  const handleNext = async () => {
    ...
    if (question.id === 'height_weight') {
      ...
      if (!h || !w) { setError("Please enter both your height and weight."); return; }
  ...
  // ── RENDER: DIET IMAGES (vegetables_fruits) ───────────────────────────────
  const renderDietImages = () => {
    if (question?.id !== 'vegetables_fruits') return null;
    return (
      <div className="grid grid-cols-2 gap-3 mb-5">
        {[
          { src: '/assets/images/diets.png', caption: 'Vegetables, fruits & berries protect blood sugar and blood pressure.' },
          { src: '/assets/images/drinks.png', caption: 'What you drink matters too — water and low-sugar drinks protect metabolic health.' },
        ].map(({ src, caption }) => (
          <div key={src} className="overflow-hidden rounded-xl border" style={{ borderColor: C.border }}>
            <div className="relative w-full h-32">
              <Image src={src} alt={caption} fill className="object-cover" />
            </div>
            <div className="p-3">
              <p className="text-[11px] leading-relaxed" style={{ color: C.muted }}>{caption}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            >
              {isYesNo ? (
                <div
                  ...
                  style={{
                    backgroundColor: sel ? C.primary : isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)',
    ...
    return (
      <div className="space-y-6">
        <div className="text-center space-y-1">
          ...
          <div className="text-sm font-semibold uppercase tracking-wider" style={{ color: C.muted }}>{question.unit}</div>
        </div>
        <div className="relative pt-2 pb-8 px-2">
          <div className="h-3 rounded-full overflow-hidden" style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)' }}>
            <div
              ...
              style={{ width: `${pct}%`, background: `linear-gradient(90deg, #0FBB7D, #059669)` }}
            />
          </div>
          <input
            type="range"
  ...
  if (loading && !question) {
    return (
      <div className="min-h-screen flex items-center justify-center transition-colors duration-300" style={{ backgroundColor: C.bg }}>
        <div className="text-center space-y-6">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: C.primary }}>Preparing Assessment</p>
            <h2 className="text-xl font-bold" style={{ color: C.text }}>Personalising your questions…</h2>
            <p className="text-sm mt-1" style={{ color: C.muted }}>This takes just a moment</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ backgroundColor: C.bg }}>
        <div className="text-center space-y-4">
          ...
          <p className="text-sm" style={{ color: C.muted }}>Unable to load questions. Please refresh and try again.</p>
          <button
      ...
      <div className="max-w-2xl mx-auto px-5 py-10 lg:px-8">
        ...
        <div className="mb-7 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold tabular-nums" style={{ color: C.muted }}>{progress}%</span>
          </div>
        ...
        >
          ...
          <div className="flex items-start gap-4 p-6 border-b" style={{ borderColor: C.border }}>
            <div
              ...
              style={{ backgroundColor: question?.aiGenerated ? 'rgba(139,92,246,0.12)' : C.primaryFaint, color: question?.aiGenerated ? '#8B5CF6' : C.primary }}
            >
          ...
          <div className="p-6 space-y-4">
            ...
            {/* Diet images for vegetables question */}
            {renderDietImages()}
        ...
        <div className="mt-7 flex items-center justify-between">
          <button
            ...
            style={{ color: C.muted, backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)' }}
          >
          ...
          >
            {submitting ? (
              ...
              <>{groqService.getQuestionCount() >= groqService.getMaxQuestions() ? 'View Results' : 'Continue'}<ChevronRight className="w-4 h-4" /></>
            )}
          </button>
        </div>
