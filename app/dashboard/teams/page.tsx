...
interface ProgramEnrollment {
  $id: string; $createdAt: string;
  programId: string; companyId: string; userId: string;
  programTitle: string; programDesc: string;
  programCategory: string; programPriority: string;
  targetRisk: string; enrolledAt: string; status: ProgramStatus;
}
...
function categoryColor(cat: string): string {
  const map: Record<string, string> = {
    screening: "#EF4444", fitness: "#10B981", nutrition: "#F59E0B",
    education: "#3B82F6", mental_health: "#8B5CF6", chronic_disease: "#EC4899", preventive: "#0D9488",
  };
...
// ─── PROGRAM CARD ─────────────────────────────────────────────────────────────
function ProgramCard({ enrollment, onMarkComplete }: {
  enrollment: ProgramEnrollment;
  onMarkComplete: (id: string) => void;
}) {
  const { surface, accentColor } = useTheme();
  const [completing, setCompleting] = useState(false);
  const color       = categoryColor(enrollment.programCategory);
  ...
  return (
    <div style={{
      ...
      opacity: isCompleted ? 0.7 : 1, transition: "opacity 0.2s",
    }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
        <div style={{ width: 32, height: 32, background: `${color}12`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          {categoryIcon(enrollment.programCategory, 15)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 3 }}>
            ...
            {isCompleted ? (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", fontSize: 10, fontWeight: 700, background: "rgba(34,197,94,0.10)", color: "#22c55e" }}>
                <CheckCircle size={9} />Done
              </span>
            ) : (
              <span style={{ padding: "2px 8px", fontSize: 10, fontWeight: 700, background: `${priorityColor(enrollment.programPriority)}18`, color: priorityColor(enrollment.programPriority) }}>
      ...
      <p style={{ margin: "0 0 12px", fontSize: 12, color: surface.muted, lineHeight: 1.65 }}>
        ...
        {!isCompleted && (
          <button
            ...
            style={{
              ...
              cursor: completing ? "default" : "pointer", opacity: completing ? 0.6 : 1, transition: "all 0.15s",
            }}
          >
            {completing ? <><Loader2 size={10} className="animate-spin" />Saving…</> : <><CheckCircle size={10} />Mark Complete</>}
          </button>
        )}
      </div>
    </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        borderBottom: `1px solid ${surface.border}`, cursor: "pointer", transition: "background 0.15s ease",
        animation: `rowIn 0.35s ease both`, animationDelay: `${index * 55}ms`,
      }}
    >
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {!programsLoading && activePrograms.length > 0 && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: completedPrograms.length > 0 ? 20 : 0 }}>
              {activePrograms.map(e => <ProgramCard key={e.$id} enrollment={e} onMarkComplete={handleMarkComplete} />)}
            </div>
          </>
        )}
        ...
        {!programsLoading && completedPrograms.length > 0 && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {completedPrograms.map(e => <ProgramCard key={e.$id} enrollment={e} onMarkComplete={handleMarkComplete} />)}
            </div>
          </>
        )}
      </div>
  ...
  const loadTeams = useCallback(async (userId: string) => {
    try {
      ...
      const res = await db.listDocuments(USERS_DB_ID, COMPANY_MEMBERS_COLLECTION_ID, [
        Query.equal("userId", userId), Query.notEqual("status", "removed"),
        ...
        {!selected && (
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <button onClick={handleRefresh} disabled={refreshing} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 14px", background: surface.surface, border: `1px solid ${surface.border}`, color: surface.muted, fontSize: 12, fontWeight: 600, cursor: "pointer", opacity: refreshing ? 0.5 : 1, transition: "opacity 0.2s" }}>
            ...
            {!loading && teams.length > 0 && (
              <>
                <div style={{ display: "flex", alignItems: "center", padding: "8px 16px", borderBottom: `1px solid ${surface.border}`, background: isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)" }}>
                  ...
                  <div style={{ flex: 1 }}><span style={{ fontSize: 10, fontWeight: 700, color: surface.subtle, textTransform: "uppercase" as const, letterSpacing: "0.1em" }}>Organisation</span></div>
                  <div style={{ paddingRight: 34, textAlign: "right" as const }}><span style={{ fontSize: 10, fontWeight: 700, color: surface.subtle, textTransform: "uppercase" as const, letterSpacing: "0.1em" }}>Joined</span></div>
                </div>
                <div style={{ border: `1px solid ${surface.border}`, borderTop: "none" }}>
                  {teams.map((team, i) => <TeamRow key={team.$id} team={team} index={i} onClick={() => setSelected(team)} />)}
                </div>
