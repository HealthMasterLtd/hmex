...
import {
  getCompanyByOwner, getCompanyMembers, type Company,
...
import {
  Sparkles, RefreshCw, ChevronDown, ChevronRight, ChevronUp,
...
interface MemberWithRisk {
  ...
  assessment: { diabetesLevel: string; diabetesScore: number; hypertensionLevel: string; hypertensionScore: number; profileGender: string; profileAge: string; profileBmi: string; $createdAt: string } | null;
}

// ─── HELPERS ─────────────────────────────────────────────────────────────────
function Skeleton({ w, h, r = 4 }: { w: string | number; h: number; r?: number }) {
  const { isDark } = useTheme();
  return <div className="animate-pulse" style={{ width: w, height: h, borderRadius: r, background: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)" }} />;
}
...
// ─── PROGRAM CARD ─────────────────────────────────────────────────────────────
function ProgramCard({ prog, c, accentColor }: { prog: AiProgramSuggestion; c: any; accentColor: string }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      ...
      {/* Top accent bar */}
      <div style={{ height: 3, background: prog.color }} />

      <div style={{ padding: "18px 20px" }}>
        {/* Header row */}
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 2, background: `${prog.color}15`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
            {prog.icon}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 3 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 800, color: c.text, lineHeight: 1.3 }}>{prog.title}</p>
              <span style={{ padding: "3px 8px", borderRadius: 2, background: priorityBg(prog.priority), color: priorityColor(prog.priority), fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.07em", flexShrink: 0 }}>
                {prog.priority}
              </span>
            </div>
            <p style={{ margin: 0, fontSize: 11, color: accentColor, fontWeight: 700, fontStyle: "italic" }}>{prog.tagline}</p>
          </div>
        </div>

        <p style={{ margin: "0 0 12px", fontSize: 12, color: c.muted, lineHeight: 1.6 }}>{prog.description}</p>
        ...
        {/* Urgency */}
        <div style={{ padding: "7px 10px", background: `${prog.color}0C`, border: `1px solid ${prog.color}22`, borderRadius: 2, marginBottom: 12 }}>
          <p style={{ margin: 0, fontSize: 11, color: prog.color, fontWeight: 600 }}><Zap size={11} style={{ display: "inline", marginRight: 4 }} />Why now: {prog.urgency}</p>
        </div>

        {/* Expand toggle */}
        <button onClick={() => setExpanded(v => !v)}
          style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, color: accentColor, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
        ...
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }}
            ...
            <div style={{ padding: "0 20px 20px", borderTop: `1px solid ${c.border}` }}>
              ...
              {/* Steps */}
              <p style={{ margin: "16px 0 10px", fontSize: 11, fontWeight: 800, color: c.muted, textTransform: "uppercase", letterSpacing: "0.07em" }}>Implementation Steps</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {prog.steps.map((step, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                    <div style={{ width: 22, height: 22, borderRadius: "50%", background: `${prog.color}18`, border: `1.5px solid ${prog.color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: prog.color, flexShrink: 0 }}>{i + 1}</div>
                    <div style={{ flex: 1 }}>
                      <span style={{ fontSize: 10, fontWeight: 700, color: prog.color }}>{step.week} · {step.responsible}</span>
              ...
              {/* KPIs */}
              <p style={{ margin: "14px 0 8px", fontSize: 11, fontWeight: 800, color: c.muted, textTransform: "uppercase", letterSpacing: "0.07em" }}>Success Metrics</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {prog.kpis.map((k, i) => (
                  <span key={i} style={{ padding: "4px 10px", background: `${prog.color}10`, color: prog.color, borderRadius: 2, fontSize: 11, fontWeight: 600 }}>{k}</span>
                ))}
              </div>

              {/* Resources */}
              <p style={{ margin: "14px 0 8px", fontSize: 11, fontWeight: 800, color: c.muted, textTransform: "uppercase", letterSpacing: "0.07em" }}>Resources Needed</p>
...
// ─── AI CHAT PANEL ────────────────────────────────────────────────────────────
function AiChatPanel({ snapshot, c, accentColor, onClose }: { snapshot: WorkforceSnapshot; c: any; accentColor: string; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 40 }}
      style={{ position: "fixed", right: 16, bottom: 80, width: 360, maxHeight: 520, background: c.surface, border: `1px solid ${c.border}`, borderRadius: 4, boxShadow: "0 20px 60px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", zIndex: 100, overflow: "hidden" }}>

      {/* Header */}
      <div style={{ padding: "12px 16px", borderBottom: `1px solid ${c.border}`, display: "flex", alignItems: "center", gap: 8, background: `${accentColor}0C` }}>
        <Sparkles size={15} style={{ color: accentColor }} />
        <span style={{ fontSize: 13, fontWeight: 800, color: c.text, flex: 1 }}>AI Health Advisor</span>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: c.muted, padding: 2 }}><X size={15} /></button>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: "85%", padding: "9px 12px", borderRadius: msg.role === "user" ? "12px 12px 2px 12px" : "12px 12px 12px 2px", background: msg.role === "user" ? accentColor : c.bg || "rgba(0,0,0,0.04)", color: msg.role === "user" ? "white" : c.text, fontSize: 12, lineHeight: 1.6 }}>
              {msg.text}
            </div>
          </div>
        ))}
      ...
      <div style={{ padding: "10px 12px", borderTop: `1px solid ${c.border}`, display: "flex", gap: 8 }}>
        ...
        <button onClick={send} disabled={!input.trim() || loading}
          style={{ padding: "8px 12px", background: accentColor, border: "none", borderRadius: 2, cursor: "pointer", color: "white", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Send size={13} />
        </button>
      </div>
    </motion.div>
...
export default function EmployerProgramsPage() {
  ...
  const loadData = useCallback(async (uid: string) => {
    ...
    try {
      ...
      if (!co) {
        ...
        if (p?.companyName) {
          co = { $id: p.companyId || uid, $createdAt: "", name: p.companyName, ownerId: uid, size: p.companySize || "", industry: p.industry || "", inviteCount: 0 } as Company;
        }
      }
      setCompany(co);
      ...
      const withRisks: MemberWithRisk[] = await Promise.all(
        rows.map(async (m) => {
          ...
          return {
            ...m,
            assessment: a ? {
              diabetesLevel: a.diabetesLevel, diabetesScore: Number(a.diabetesScore),
              hypertensionLevel: a.hypertensionLevel, hypertensionScore: Number(a.hypertensionScore),
              profileGender: a.profileGender, profileAge: a.profileAge,
              profileBmi: a.profileBmi, $createdAt: a.$createdAt,
            } : null,
          };
        })
      );
  ...
  const runAi = async (snap: WorkforceSnapshot) => {
    setAiLoading(true);
    setAiError(null);
    try {
      const result = await employerProgramsAiService.generateFullAnalysis(snap);
      setAiData(result);
    } catch (e: any) {
      setAiError("AI analysis failed. Showing fallback suggestions.");
  ...
  const handleRefresh = () => { if (user) loadData(user.id); };
  ...
  return (
    <EmployerLayout>
      <div style={{ paddingBottom: 80 }}>
          ...
          <div>
            ...
            {company && (
              <p style={{ margin: "4px 0 0", fontSize: 12, color: c.muted, display: "flex", alignItems: "center", gap: 5 }}>
                ...
                {lastRefresh && <span style={{ fontSize: 10, opacity: 0.55 }}> · {lastRefresh.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>}
              </p>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={handleRefresh} disabled={loading || aiLoading}
              style={{ padding: "8px 14px", background: "transparent", border: `1px solid ${c.border}`, color: c.muted, borderRadius: 2, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600 }}>
              <RefreshCw size={13} className={(loading || aiLoading) ? "animate-spin" : ""} />Refresh
            </button>
        ...
        <AnimatePresence>
          {!loading && aiData?.urgentMessage && (
            <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 2, marginBottom: 20 }}>
        ...
        {/* ── AI Headline ──────────────────────────────────────────────────── */}
        {!loading && (
          <div style={{ padding: "14px 18px", background: isDark ? `${accentColor}0A` : `${accentColor}06`, border: `1px solid ${accentColor}20`, borderRadius: 2, marginBottom: 24, display: "flex", alignItems: "flex-start", gap: 12 }}>
            <div style={{ width: 32, height: 32, borderRadius: 2, background: `${accentColor}18`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Sparkles size={15} style={{ color: accentColor }} />
            </div>
            <div>
              <p style={{ margin: "0 0 2px", fontSize: 11, fontWeight: 800, color: accentColor, textTransform: "uppercase", letterSpacing: "0.07em" }}>AI Analysis</p>
              {aiLoading
                ? <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Loader2 size={13} className="animate-spin" style={{ color: accentColor }} /><span style={{ fontSize: 12, color: c.muted }}>Analysing your workforce data…</span></div>
                : <p style={{ margin: 0, fontSize: 13, color: c.text, lineHeight: 1.5 }}>{aiData?.headline || "Building your personalised health program recommendations…"}</p>
              }
            </div>
          </div>
        )}
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: `1px solid ${c.border}`, paddingBottom: 0 }}>
            {([
              ...
              <button key={t.key} onClick={() => setTab(t.key)}
                style={{ padding: "8px 16px", background: "none", border: "none", borderBottom: tab === t.key ? `2px solid ${accentColor}` : "2px solid transparent", marginBottom: -1, cursor: "pointer", fontSize: 12, fontWeight: tab === t.key ? 800 : 600, color: tab === t.key ? accentColor : c.muted, display: "flex", alignItems: "center", gap: 5, transition: "color 0.15s" }}>
                {t.icon}{t.label}
              </button>
            ))}
          </div>
          ...
          <>
            {aiLoading || loading ? (
              ...
              <div style={{ padding: "32px 24px", textAlign: "center", background: c.surface, border: `1px solid ${c.border}`, borderRadius: 2 }}>
                ...
                <button onClick={() => snapshot && runAi(snapshot)}
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    ...
                    <div>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, color: accentColor, textTransform: "uppercase", letterSpacing: "0.06em" }}>{tip.category.replace("_"," ")}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ padding: "8px 10px", background: `${accentColor}0C`, border: `1px solid ${accentColor}20`, borderRadius: 2 }}>
                    <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: accentColor }}><Zap size={11} style={{ display: "inline", marginRight: 4 }} />Do this today: {tip.actionable}</p>
                  </div>
                </motion.div>
              ))
            }
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12 }}>
                    {strat.how.map((step, j) => (
                      <div key={j} style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                        <div style={{ width: 18, height: 18, borderRadius: "50%", background: `${accentColor}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: accentColor, flexShrink: 0, marginTop: 1 }}>{j + 1}</div>
